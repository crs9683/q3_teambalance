...
int shuffleTeamsInProgress = 0; // 0 means not in progress, 1 means in progress
...
void BroadcastTeamChange( gclient_t *client, team_t oldTeam )
{
	...
	if ( client->sess.sessionTeam == TEAM_RED ) {
		G_BroadcastServerCommand( clientNum, va("cp \"%s" S_COLOR_WHITE " joined the " S_COLOR_RED "^1RED" S_COLOR_WHITE " ^7team.\n\"",
			client->pers.netname) );
	} else if ( client->sess.sessionTeam == TEAM_BLUE ) {
		G_BroadcastServerCommand( clientNum, va("cp \"%s" S_COLOR_WHITE " joined the " S_COLOR_BLUE "^4BLUE" S_COLOR_WHITE " ^7team.\n\"",
		client->pers.netname));
	} else if ( client->sess.sessionTeam == TEAM_SPECTATOR && oldTeam != TEAM_SPECTATOR ) {
		G_BroadcastServerCommand( clientNum, va("cp \"%s" S_COLOR_WHITE " joined the ^3SPECTATORS.\n\"",
		client->pers.netname));
...
qboolean IsBot(int clientNum) {
	// Assuming g_entities[clientNum].r.svFlags & SVF_BOT indicates a bot
	return (g_entities[clientNum].r.svFlags & SVF_BOT);
}

static qboolean AllowTeamSwitch(int clientNum, team_t newTeam) {
	// Allow team switch if ShuffleTeams is in progress
	if (shuffleTeamsInProgress) {
		return qtrue;
	}

	if (IsBot(clientNum)) {
		return qtrue; // Allow bots to switch teams freely
	}
	...
	if (g_teamForceBalance.integer) {
		...
		// Allow a maximum difference of one player between the teams
		if (newTeam == TEAM_RED && counts[TEAM_RED] - counts[TEAM_BLUE] >= 1) {
			trap_SendServerCommand(clientNum, "cp \"^1RED ^7team has too many players.\n\"");
			return qfalse; // ignore the request
		}

		if (newTeam == TEAM_BLUE && counts[TEAM_BLUE] - counts[TEAM_RED] >= 1) {
			trap_SendServerCommand(clientNum, "cp \"^4BLUE ^7team has too many players.\n\"");
			return qfalse; // ignore the request
		}

		// If the teams are even in number of players, prevent switching to the team with the greater score
		if (counts[TEAM_RED] == counts[TEAM_BLUE]) {
			if (newTeam == TEAM_RED && redScore > blueScore) {
				trap_SendServerCommand(clientNum, "cp \"^1RED ^7team has the lead.\n\"");
				return qfalse; // ignore the request
			}
			if (newTeam == TEAM_BLUE && blueScore > redScore) {
				trap_SendServerCommand(clientNum, "cp \"^4BLUE ^7team has the lead.\n\"");
				return qfalse; // ignore the request
			}
		}

		// It's ok, the team we are switching to has less or the same number of players and a lower or equal score
	}
...
static qboolean ValidVoteCommand( int clientNum, char *command ) 
{
	...
	*s = '\0';
	...
	if (Q_stricmp(buf, "map") == 0) {
		char map[256];
		Q_strncpyz(map, command, sizeof(map));

		if (!G_MapExist(map) || !MapInRotation(map)) {
			trap_SendServerCommand(clientNum, "print \"^3Map not found.\n\"");
			return qfalse;
		}
...
void Cmd_CallVote_f(gentity_t* ent) {
	...
	// Check if enough time has passed since the player joined
	if (currentTime - ent->client->pers.joinTime < 5000 ) { // 5 seconds delay
		trap_SendServerCommand(ent - g_entities, "print \"You must wait 5 seconds after joining before calling a vote.\n\"");
		return;
	}
	...
	// Update last callvote time
	lastCallVoteTime = trap_Milliseconds();
}
