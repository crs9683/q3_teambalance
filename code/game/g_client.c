...
void ClientBegin(int clientNum) {
	...
	client->pers.joinTime = level.time;
	...
	if (client->sess.sessionTeam != TEAM_SPECTATOR) {
		...
		if (g_gametype.integer != GT_TOURNAMENT && !client->pers.inGame) {
			// Determine team name with color codes
			if (client->sess.sessionTeam == TEAM_RED) {
				teamColorName = "^1RED^7";
			}
			else if (client->sess.sessionTeam == TEAM_BLUE) {
				teamColorName = "^4BLUE^7";
			}
			else {
				teamColorName = "^7UNKNOWN^7";
			}

			// Broadcast message with team info and color
			G_BroadcastServerCommand(-1, va("print \"%s^7 entered the game (%s^7)\n\"", client->pers.netname, teamColorName));
		}
	}
