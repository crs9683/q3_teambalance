...
int autoTeamBalanceActive = 1; // 1 for active, 0 for inactive
...
void TrackPlayerFrags(void) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		// Assuming 'frags' is the field tracking player frags
		level.clients[i].pers.frags = level.clients[i].ps.persistant[PERS_SCORE];
	}
}
...
void TrackPlayerTime(void) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		// Assuming 'enterTime' is the field for when the player joined
		level.clients[i].pers.timeSpent = level.time - level.clients[i].pers.enterTime;
	}
}
...
==============
*/
#define TEAM_BALANCE_INTERVAL 3000 // Interval in milliseconds (3 seconds)

extern qboolean IsBot(int clientNum);

static int nextTeamBalanceTime = 0;

void AutoTeamBalance(void);

void CheckTeamBalance(void) {
	int currentTime = level.time;

	if (!autoTeamBalanceActive) {
		return; // AutoTeamBalance is temporarily disabled
	}

	if (currentTime >= nextTeamBalanceTime) {
		AutoTeamBalance();
		nextTeamBalanceTime = currentTime + TEAM_BALANCE_INTERVAL;
	}
}
...
void AutoTeamBalance(void) {
	...
	int i;
	int minFrags, minTimeSpent;
	gclient_t* playerToMove;

	// Initialize counts
	counts[TEAM_BLUE] = 0;
	counts[TEAM_RED] = 0;

	// Count human players on each team, ignoring spectators and bots
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].pers.connected != CON_CONNECTED) {
			continue;
		}
	...
	// Balance teams by moving human players based on the least amount of frags and time spent
	while (counts[TEAM_BLUE] > counts[TEAM_RED] + 1) {
		minFrags = INT_MAX;
		minTimeSpent = INT_MAX;
		playerToMove = NULL;
		...
		for (i = 0; i < level.maxclients; i++) {
			...
			if (level.clients[i].sess.sessionTeam == TEAM_BLUE) {
				int frags = level.clients[i].ps.persistant[PERS_SCORE]; // Use the tracked frags
				int timeSpent = level.clients[i].pers.enterTime; // Use the tracked time spent

				// Find the player with the least frags
				if (frags < minFrags || (frags == minFrags && timeSpent < minTimeSpent)) {
					minFrags = frags;
					minTimeSpent = timeSpent;
					playerToMove = &level.clients[i];
				}
			}
		}

		if (playerToMove != NULL) {
			SetTeam(&g_entities[playerToMove - level.clients], "red");
			counts[TEAM_BLUE]--;
			counts[TEAM_RED]++;
		}
		else {
	...
	while (counts[TEAM_RED] > counts[TEAM_BLUE] + 1) {
		minFrags = INT_MAX;
		minTimeSpent = INT_MAX;
		playerToMove = NULL;
		...
		if (playerToMove != NULL) {
			SetTeam(&g_entities[playerToMove - level.clients], "blue");
			counts[TEAM_RED]--;
			counts[TEAM_BLUE]++;
		}
		else {
...
int GatherHumanPlayers(playerInfo_t* players) {
	int numPlayers = 0;
	int i;
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].pers.connected != CON_CONNECTED) {
		...
		players[numPlayers].clientNum = i;
		players[numPlayers].frags = level.clients[i].ps.persistant[PERS_SCORE];
		numPlayers++;
	}
	return numPlayers;
}
...
void ShuffleTeams(void) {
	...
	int originalAutoTeamBalanceState = autoTeamBalanceActive;
	int i, j;
	playerInfo_t players[MAX_CLIENTS];
	int numPlayers;
	playerInfo_t temp;

	// Indicate that team shuffling is in progress
	shuffleTeamsInProgress = 1;

	// Disable AutoTeamBalance
	autoTeamBalanceActive = 0;

	// Gather human players into the players array
	numPlayers = GatherHumanPlayers(players);
	...
	// Set the players to teams alternately
	for (i = 0; i < numPlayers; i++) {
		if (i % 2 == 0) {
			SetTeam(&g_entities[players[i].clientNum], "red");
		}
		else {
			SetTeam(&g_entities[players[i].clientNum], "blue");
		}
	}

	// Notify all clients that teams have been shuffled
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].pers.connected == CON_CONNECTED) {
			trap_SendServerCommand(i, "cp \"Teams have been shuffled!\"");
		}
	}

	// Restore the original AutoTeamBalance state
	autoTeamBalanceActive = originalAutoTeamBalanceState;

	// Indicate that team shuffling is complete
	shuffleTeamsInProgress = 0;
}
...
static int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, team_t team ) {
	...
	if ( ent->flags & FL_DROPPED_ITEM ) {
		...
		PrintMsg( NULL, "%s" S_COLOR_WHITE " ^3returned the %s flag!\n", 
...
#ifdef MISSIONPACK
	if( g_gametype.integer == GT_1FCTF ) {
		PrintMsg (NULL, "%s" S_COLOR_WHITE " ^3got the flag!\n", other->client->pers.netname );
...
#endif
		PrintMsg (NULL, "%s" S_COLOR_WHITE " ^3got the %s flag!\n",
