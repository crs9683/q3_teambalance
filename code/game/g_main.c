...
int lastCallVoteTime = 0; // Define the variable
int lastMapRotationTime = 0; // Define the variable
...
static void G_InitGame(int levelTime, int randomSeed, int restart) {
	...
	if (restart) {
		G_Printf("G_InitGame: Server is restarting\n");
	}
	else {
		G_Printf("G_InitGame: Server is starting up\n");
	}

	// Initialize last map rotation time
	lastMapRotationTime = levelTime;
	...
	G_Printf("G_InitGame: Initialization complete\n");
}
...
static void G_ShutdownGame(int restart)
{
	...
	if (restart) {
		G_Printf("G_ShutdownGame: Server is restarting\n");
	}
	else {
		G_Printf("G_ShutdownGame: Server is shutting down\n");
	}
	...
	G_WriteSessionData();
	G_Printf("G_ShutdownGame: Session data written\n");

	if (trap_Cvar_VariableIntegerValue("bot_enable")) {
		G_Printf("G_ShutdownGame: Shutting down bots\n");
		BotAIShutdown(restart);
	}

	G_Printf("==== ShutdownGame Complete ====\n");
}
...
static void G_RunFrame( int levelTime ) {
	...
	// Check if the game is in a warmup period
	if (level.warmupTime == 0) {
		CheckTeamBalance();
	}
